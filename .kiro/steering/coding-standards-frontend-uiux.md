...
- TUYỆT ĐỐI KHÔNG hardcode string hiển thị cho user
- Client component: `useTranslations('namespace')`
- Server component: `await getTranslations('namespace')`
- Thêm key mới: phải thêm vào cả `messages/ja.json` và `messages/en.json`
- Navigation: dùng `useRouter` từ `@/lib/navigation` (next-intl wrapper) — KHÔNG dùng từ `next/navigation`
...
- `404` → set về `null`, KHÔNG set `error`
- `401` → redirect về login (Axios interceptor)
- `409` → set `error` với message cụ thể
- `422`/`400` → set `error` với message từ response body
